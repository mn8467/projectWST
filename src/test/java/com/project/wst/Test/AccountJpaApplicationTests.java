 ...
 public class AccountJpaApplicationTests {
    ...
    void insertTestData() {
     ...
     void findAllTest() { // 저장된 데이터 모두를 Spring JPA에 미리 구현된 findAll 명령을 통해 불러온다
         List<Member> accounts = accountsRepository.findAll();
         for(Member u : accounts) log.info("[FindAll]: " + u.getUserCode() + " | " +u.getUserName());
     }

     @Test
     void find2ByNameTest() { // Like 검색으로 2개만 값을 가져오는 내가 작성한 명령을 실행해본다
         List<Member> accounts = accountsRepository.findUserListByUserCode(1);
         for(Member u : accounts) log.info("[FindSome]: "  + u.getUserCode() + " | " +u.getUserName());
     }
